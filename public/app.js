$(document).ready(function() {
  $.getJSON( "/weather", function(data) {
    console.log('datatype: ', typeof(data));
    console.log(data[0]);
    var data1 = data[0].data;
    $('#date-time').html(moment.unix(data1.currently.time).format("MM/DD/YYYY h:mm:ss A"), '\n');
    $('#current-temp').html(Math.round(data1.currently.temperature));
    $('#current-condition').html(data1.currently.summary);
    $('#today-high').html(Math.round(data1.daily.data[0].temperatureMax));
    $('#today-low').html(Math.round(data1.daily.data[0].temperatureMin));
    $('#next-hour-forecast').html(data1.minutely.summary);
    $('#today-forecast').html(data1.hourly.summary);
    $('#week-forecast').html(data1.daily.summary);

    for (i = 1; i < data1.daily.data.length; i++) {
      if (i > 1) {
        var dayOfWeek = moment.unix(data1.currently.time + (i * 86400)).format("dddd");
      } else {
        var dayOfWeek = "Tomorrow";
      }
      $('#week-daily-forecast').append("<li class='weekday'>" + "<p><strong>" + dayOfWeek + ": </strong>" + data1.daily.data[i].summary + "</p>" + "<p>High: <span class='high-temp'>" + Math.round(data1.daily.data[i].temperatureMax) + "</span>&#8457; - Low: <span class='low-temp'>" + Math.round(data1.daily.data[i].temperatureMin) + "</span>&#8457;</p></li>");
    }

    for (i = 0; i < 9; i++) {
      // limited to 9 results, to display all available switch 9 with data.hourly.data.length
      var info = data1.hourly.data[i];
      $('#hourly-list').append("<li><h4>" + moment.unix(info.time).format("ddd, hA") + "</h4><h6>" + "Summary: " + info.summary + "</h6><p>Temp: " + Math.round(info.temperature) + "&#8457;</p></li>");
    }

    for (i = 0; i < 25; i++) {
      // limited to 25 results, to display all available switch 25 with data.minutely.data.length
      var minInfo = data1.minutely.data[i];
      $('#minutely-list').append("<li><p>" + moment.unix(minInfo.time).format("h:mm A") + " - " + (minInfo.precipProbability * 100) + "%</p></li>");
    }
  });
});
